class Polygon {
    ...
    static break({segments: segs1}, {segments: segs2}) {
    ...
    drawSegments(ctx) {
        this.segments.forEach(s => s.draw(ctx, {color: getRandomColor(), width: 5}));
    }

    draw(ctx, {stroke = 'blue', lineWidth = 2, fill = 'rgba(0, 0, 255, .3)'} = {}) {
        ctx.beginPath();
        ctx.fillStyle = fill;
